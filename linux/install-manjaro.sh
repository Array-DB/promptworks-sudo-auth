...
echo "IMPORTANT: KEEP ANOTHER ROOT SHELL OPEN WHILE TESTING PAM."
pacman -S --needed --noconfirm base-devel pam openssl
...
install -Dm755 "$ROOT/pam/pam_promptworks.so" /usr/lib/security/pam_promptworks.so
...
"$ROOT/install-pam-order.sh" system-auth
...
echo "PromptWorks single-device-bound OFFLINE time-match PAM installed. No runtime server, LAN, Internet, polling, or notification delivery is required."
